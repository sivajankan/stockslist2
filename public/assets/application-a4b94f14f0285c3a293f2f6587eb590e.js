  ...
  function show(e) {
    if (zooming) {
      return false;
		...
		//var content_div = $($(this.parentElement).attr('href'));
		var content_div = $($(this).data('fancydiv'));
...
(function() {
  ...
  stock_image_url = function(id, day_text) {
    var url;
    url = $('#' + id).attr('src');
  ...
  window.load_yahoo_info = function(url) {
    $.ajax({
      url: url,
      success: function(data) {
        return $("div#stock_info").html(data);
      },
      failure: function(data) {
        return failure("Yahoo data fetch failed");
      }
    });
    return $.ajax({
      url: url + "&group=2",
      success: function(data) {
        ...
        return $($('#period_selection td')[0]).css('text-decoration', 'underline');
      },
      failure: function(data) {
        return failure("Yahoo data fetch failed");
      }
  ...
  $().ready(function() {
    return $('#period_selection td').on('click', function(evt) {
      evt.preventDefault();
